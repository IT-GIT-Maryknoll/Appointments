...
namespace Appointments.Pages
    {
    public class IndexModel(ILogger<IndexModel> logger,IConfiguration configuration,IAppointmentsRepository appointmentsData,IPermissionsRepository permissionsRepository):
        ...
        public void OnGet()
            ...
            if(s!=null&&s.Trim().Length>0)
                ...
                if(doc.RootElement.TryGetProperty("lblDate",out e))
                    { lblDate=e.GetString().Substring(0,7); bFilter="true"; }
                if(doc.RootElement.TryGetProperty("hdnResident",out e))
                    { hdnResident=e.GetString(); bFilter="true"; }
                if(doc.RootElement.TryGetProperty("txtResident",out e))
                    { txtResident=e.GetString(); bFilter="true"; }
                if(doc.RootElement.TryGetProperty("hdnDoctor",out e))
                    { hdnDoctor=e.GetString(); bFilter="true"; }
                if(doc.RootElement.TryGetProperty("txtDoctor",out e))
                    { txtDoctor=e.GetString(); bFilter="true"; }
                if(doc.RootElement.TryGetProperty("cmbApptType",out e))
                    { cmbApptType=e.GetString(); bFilter="true"; }
                GetFilterIn();
                }
            }
        ...
        public JsonResult OnGetDeleteAppointment(int id)
            ...
            if(oExisting!=null)
                {
                if((TempData["Message"]==null||(TempData["Message"]?.ToString()?.Length??0)==0)&&sTempMsg!=null&&sTempMsg.Trim().Length>0)
                    TempData["Message"]=sTempMsg;
                else
                    TempData["Message"]+=sTempMsg;
                sTempMsg="";
                }
            ...
            if(bRet)
                SaveAudit(GetUserName(),GetIpValue(),"Appointments","Delete",sAuditDesc);
        ...
        public StatusCodeResult OnPostSaveAppointment()
            {
            string sAuditDesc = "";
            AppointmentDto oExisting = null;
            string sMsg = "";
            int iTemp = 0;
            DateTime dTemp = new DateTime();
            AppointmentDto appointment = new AppointmentDto();
            if(int.TryParse(Request.Form["hdnEventID"],out iTemp))
                appointment.ID=iTemp;
            if(appointment.ID>0)
                {
            ...
            if(DateTime.TryParse(Request.Form["txtMDate"]+" "+Request.Form["txtMTime"],out dTemp))
                appointment.ApptTime=dTemp;
            ...
            if(int.TryParse(Request.Form["hdnMRes"],out iTemp))
                appointment.ResidentKey=iTemp;
            ...
            if(int.TryParse(Request.Form["hdnMDoc"],out iTemp))
                appointment.DoctorKey=iTemp;
            ...
            if(int.TryParse(Request.Form["cmbMAddr"],out iTemp))
                appointment.LocationID=iTemp;
            ...
            if(int.TryParse(Request.Form["hdnMDriver"],out iTemp))
                appointment.DriverKey=(iTemp==0 ? null : iTemp);
            ...
            if(int.TryParse(Request.Form["cmbMCar"],out iTemp))
                appointment.CarNum=(iTemp==0 ? null : iTemp);
            ...
            if(int.TryParse(Request.Form["cmbMPrep"],out iTemp))
                appointment.PrepID=(iTemp==0 ? null : iTemp);
            ...
            if(bRet)
                SaveAudit(GetUserName(),GetIpValue(),"Appointments",appointment.ID==0 ? "Create" : "Update",sAuditDesc);
            if((TempData["Message"]==null||(TempData["Message"]?.ToString()?.Length??0)==0)&&sMsg!=null&&sMsg.Trim().Length>0)
                TempData["Message"]=sMsg;
            else
                TempData["Message"]+=sMsg;
